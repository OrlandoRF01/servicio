...
app.get('/servicio/:id', (req, res) => {
    ...
    const sparePart = data.servicio.find((sparePart) => sparePart.id === id);
    res.json(sparePart);
});
...
app.post('/servicio', (req, res) => {
    ...
    const body = req.body;
    const newSparePart = {
        id: data.servicio.length + 1,
        ...body,
    };
    data.servicio.push(newSparePart);
    writeData(data);
...
app.put('/servicio/:id', (req, res) => {
    ...
    const body = req.body;
    const id = parseInt(req.params.id);
    const sparePartIndex = data.servicio.findIndex((sparePart) => sparePart.id === id);
    data.servicio[sparePartIndex] = {
        ...data.servicio[sparePartIndex],
        ...body,
    };
...
app.delete('/servicio/:id', (req, res) => {
    ...
    const id = parseInt(req.params.id);
    const sparePartIndex = data.servicio.findIndex((sparePart) => sparePart.id === id);
    data.servicio.splice(sparePartIndex, 1);
    writeData(data);
